      ...
      for (uint column = 0; column < DEFAULT_SIZE; column++) {
         ...
         for (uint startingValue = 0; startingValue < SIZE_ALL_VALUES;
            ...
            for (uint testValue = 0; testValue < SIZE_ALL_VALUES; testValue++) {
               // if the square is not black and we are trying to add a blank,
               // lightbulb or a mark check move needs to be true
...
bool test_game_play_move() {
   ...
   for (uint row = 0; row < DEFAULT_SIZE; row++) {
      for (uint column = 0; column < DEFAULT_SIZE; column++) {
         ...
         for (uint iPlayableValue = 0; iPlayableValue < size_allPlayableValues;
              iPlayableValue++) {
            ...
            for (uint iSquare = 0; iSquare < size_allModifiableSquares;
               ...
               // this is to modify the flags of the current square after the
               // play move
               for (uint iAjacentSquare = 0;
                    iAjacentSquare < size_allAjacentSquares; iAjacentSquare++) {
                  // set ajacent square
                  game_play_move(testGame, ajacent_row, ajacent_column,
                                 allAjacentSquares[iAjacentSquare]);
                  square ajacentSquareState =
                      game_get_state(testGame, ajacent_row, ajacent_column);
                  // set current square
   ...
   for (uint row = 0; row < DEFAULT_SIZE; row++) {
      for (uint column = 0; column < DEFAULT_SIZE; column++) {
         ...
         for (uint i = 0; i < SIZE_ALL_VALUES; i++) {
            ...
            // if it is black then check that it stays black else it should be
            // blank
